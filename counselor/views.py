...
def edit_profile(request):
    ...
    if request.method == "POST":
        ...
        updates = {
            field: request.POST.get(field, "").strip() or None
            for field in fields
        }
...
def edit_schedule(request):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request, f"{course.name} added to your {grade_key} schedule."
        )
        return redirect("counselor:edit_schedule")
    ...
    for target, cleaned_target in zip(target_names, cleaned_target_names):
        try:
            ...
            if best_match and best_match[1] >= 60:
                original_match = dataset_names[
                    cleaned_dataset_names.index(best_match[0])
                ]

                if any(
                    keyword.lower() in original_match.lower()
                    for keyword in target.split()
                ):
...
def track_application(request, app_id):
    ...
    if request.method == "POST":
        ...
        # Save or update essay draft
            ...
            if draft_id:
                draft = get_object_or_404(
                    EssayDraft, id=draft_id, user=user, application=application
                )
                draft.prompt = prompt
        ...
        application.application_type = int(
            request.POST.get("application_type", 1)
        )
        application.chance = int(request.POST.get("chance", 1))
        application.deadline = (
            parse_date(request.POST.get("deadline"))
            if request.POST.get("deadline")
            else None
        )
        application.location = int(request.POST.get("location", 1))
        application.rec_letter_status = (
            2 if request.POST.get("rec_letter_status") else 1
        )
        application.general_questions_status = (
            2 if request.POST.get("general_questions_status") else 1
        )
        application.grade_report_status = (
            2 if request.POST.get("grade_report_status") else 1
        )
        application.SAT_ACT_score_status = (
            2 if request.POST.get("SAT_ACT_score_status") else 1
        )
        application.scholarship_application_status = (
            2 if request.POST.get("scholarship_application_status") else 1
        )
        application.FAFSA_application_status = (
            2 if request.POST.get("FAFSA_application_status") else 1
        )
    ...
    return render(
        ...
        {"application": application, "essay_drafts": essay_drafts},
    )
